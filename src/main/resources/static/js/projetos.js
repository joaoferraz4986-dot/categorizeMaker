...
const icon = {
  ...
  itemPhoto:
    '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><rect x="3" y="4" width="18" height="16" rx="2"/><circle cx="8.5" cy="9" r="1.5"/><path d="m21 15-5-5L5 20"/></svg>',
...
function filtered() {
  ...
  return state.projects.filter((project) => {
    ...
    const matchesQuery =
      !query ||
      `${project.nome} ${project.descricao}`.toLowerCase().includes(query);
...
function renderPicker(selected = []) {
  if (selected.length) {
    state.quantities = new Map(
      selected.map((item) => [item.idItem, item.quantidadeUsada]),
    );
  }
  ...
  const items = state.items.filter(
    (item) =>
      !query || `${item.nome} ${item.tipo || ""}`.toLowerCase().includes(query),
  );
  ...
  `,
        ...
        .join("")
    : '<p class="item-picker-empty">Nenhum item encontrado.</p>';
}
...
function openView(id) {
  ...
  $("#view-project-category").textContent = categoryLabel(
    project.categoriaProjeto || project.categoria,
  );
  ...
  $("#view-project-items").innerHTML = project.itens?.length
    ? project.itens
        .map(
          (i) =>
            `<div class="linked-item"><span>${esc(i.nome)}</span><strong>${i.quantidadeUsada} un.</strong></div>`,
        )
        .join("")
...
function initProjects() {
  ...
  document.querySelectorAll(".cat-tab").forEach((tab) => {
    tab.onclick = () => {
      state.filter =
        tab.dataset.cat === "Todos" ? "TODOS" : tab.dataset.cat.toUpperCase();
      document
  ...
  $("#project-items-picker").onclick = (e) => {
    ...
    const delta = e.target.classList.contains("qty-plus")
      ? 1
